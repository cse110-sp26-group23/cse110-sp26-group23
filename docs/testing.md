...
The JSDoc reference is generated from `source/js/` into `docs/api/` and published alongside the game so it is reachable at `https://<site>/api/`. The directory is a build artifact: gitignored, regenerated by the deploy workflow on every push to `main`, and never hand-edited. Template choice and config live in [ADR-005](decisions/005-jsdoc-template.md); generation command is in [CONTRIBUTING.md](../CONTRIBUTING.md#generating-api-documentation).